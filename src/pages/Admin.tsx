...
export default function Admin() {
  ...
  type TabType = 'notices' | 'admissions' | 'facilities' | 'gallery' | 'contacts' | 'faculty' | 'academics';
  ...
  async function updateAdmissionSettings() {
    ...
    if (admissionSettings.id === 'dummy') {
      ...
      if (error) alert('Error creating settings: ' + error.message); else { alert('Settings created!'); loadAdmissionSettings(); }
    } else {
      const { error } = await supabase.from('admissions_settings').update(admissionSettings).eq('id', admissionSettings.id);
      if (error) alert('Error saving settings: ' + error.message); else alert('Admission settings updated!');
    }
  }
  ...
  async function addFaculty(e: React.FormEvent) {
    e.preventDefault();
    if (!newFaculty.name || !newFaculty.subject) return alert('Name and subject are required');
  ...
  async function updateFaculty(f: { id: string; [key: string]: any }) {
    ...
    if (error) alert('Error updating faculty: ' + error.message);
  }
  async function deleteFaculty(id: string) {
    if (!confirm('Are you sure you want to delete this faculty member?')) return;
    const { error } = await supabase.from('faculty').delete().eq('id', id);
    if (!error) setFaculty(faculty.filter(f => f.id !== id));
  }
  ...
  async function addTopStudent(e: React.FormEvent) {
    e.preventDefault();
    if (!newTopStudent.name || !newTopStudent.percentage || !newTopStudent.year) return alert('Name, Year, and Percentage are required');
  ...
  async function deleteTopStudent(id: string) {
    ...
    if (!error) setTopStudents(topStudents.filter(s => s.id !== id));
  }
  ...
  async function addGalleryItem(file: File) {
    ...
    if (data) {
      const { data: item, error: itemError } = await supabase.from('gallery_items').insert({ title: 'New Item', description: '', file_url: data.path, media_type: 'image' }).select();
      if (item) setGalleryItems([item[0], ...galleryItems]);
  ...
  async function deleteGalleryItem(id: string) {
    ...
    if (!item) return;
    const { error } = await supabase.storage.from('gallery').remove([item.file_url]);
    if (!error) {
      const { error: dbError } = await supabase.from('gallery_items').delete().eq('id', id);
      if (!dbError) setGalleryItems(galleryItems.filter(i => i.id !== id));
    }
  }
  ...
  async function updateContactSettings() {
    ...
    if (contactSettings.id === 'dummy') {
      ...
      if (error) alert('Error creating contacts: ' + error.message); else { alert('Contacts created!'); loadContactSettings(); }
    } else {
      const { error } = await supabase.from('contact_settings').update(contactSettings).eq('id', contactSettings.id);
      if (error) alert('Error saving contacts: ' + error.message); else alert('Contact settings updated!');
    }
  }
  // #endregion
        ...
        return (
          <div>
            ...
            <div className="space-y-4">
              {facilities.map(f => (
                <div key={f.id} className="p-4 border rounded-lg grid gap-2">
                  ...
                  <input type="text" placeholder="Image URL (optional)" value={f.image_url || ''} onChange={e => setFacilities(facilities.map(fac => fac.id === f.id ? {...fac, image_url: e.target.value} : fac))} className="w-full p-2 border rounded text-sm"/>
            ...
            <div className="mt-8 p-4 border rounded-lg grid gap-2">
              ...
              <input placeholder="Image URL (optional)" value={newFacility.image_url || ''} onChange={e => setNewFacility({...newFacility, image_url: e.target.value})} className="p-2 border rounded w-full"/>
              <button onClick={addFacility} className="bg-green-600 text-white px-4 py-2 rounded-lg w-fit flex items-center gap-2"><PlusCircle size={20}/> Add Facility</button>
            </div>
          </div>
        );
      case 'faculty':
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              ...
              <form onSubmit={addFaculty} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="text" placeholder="Image URL" value={newFaculty.image} onChange={e => setNewFaculty({...newFaculty, image: e.target.value})} className="p-2 border rounded"/>
                ...
                <button type="submit" className="md:col-span-2 bg-blue-600 text-white px-4 py-2 rounded font-medium flex items-center justify-center space-x-2"><PlusCircle className="w-5 h-5"/><span>Add Faculty</span></button>
              </form>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              ...
              <form onSubmit={addTopStudent} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="text" placeholder="Image URL (optional)" value={newTopStudent.image} onChange={e => setNewTopStudent({...newTopStudent, image: e.target.value})} className="p-2 border rounded"/>
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              {topStudents.map(s => (
                <div key={s.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <input value={s.image} onChange={e => setTopStudents(topStudents.map(ts => ts.id === s.id ? {...ts, image: e.target.value} : ts))} className="w-full p-2 border rounded text-sm text-gray-600" placeholder="Image URL"/>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="bg-blue-50 rounded-xl p-6 mb-8 border border-blue-100">
              ...
              <div className="flex items-center gap-4">
                <label className="flex-1 cursor-pointer bg-white border-2 border-dashed border-blue-300 rounded-lg p-6 text-center hover:bg-blue-50 transition-colors">
              ...
              {galleryItems.map(item => (
                <div key={item.id} className="relative p-3 border border-gray-200 rounded-xl bg-gray-50 shadow-sm flex flex-col gap-3 group hover:shadow-md transition-shadow">
                  <div className="relative overflow-hidden rounded-lg bg-gray-200 aspect-video">
                    <img src={item.file_url.startsWith('http') ? item.file_url : `https://xbxxtqavxhjcymeefjzk.supabase.co/storage/v1/object/public/gallery/${item.file_url}`} alt={item.title} className="w-full h-full object-cover"/>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <button onClick={async () => {
                        const { error } = await supabase.from('gallery_items').update({title: item.title}).eq('id', item.id);
                        if (error) alert(error.message); else alert('Title updated successfully!');
          ...
          <div>
            <div className="mt-8 p-4 border rounded-lg bg-blue-50">
              ...
              <div className="grid gap-3">
                ...
                <input placeholder="PDF URL (optional)" value={newNotice.pdf_url || ''} onChange={e => setNewNotice({...newNotice, pdf_url: e.target.value})} className="p-2 border rounded w-full"/>
                <button onClick={addNotice} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg w-fit flex items-center gap-2"><PlusCircle size={20}/> Add Notice</button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {notices.map(n => (
                <div key={n.id} className="p-4 border rounded-lg grid gap-2">
                  ...
                  <input value={n.pdf_url || ''} onChange={e => setNotices(notices.map(nt => nt.id === n.id ? {...nt, pdf_url: e.target.value} : nt))} className="w-full p-2 border rounded text-sm" placeholder="PDF URL (optional)"/>
                  <div className="flex gap-2 mt-2">
                    <button onClick={() => updateNotice(n)} className="bg-blue-500 text-white px-4 py-2 rounded flex items-center gap-1"><Save size={16}/> Save</button>
                    <button onClick={() => deleteNotice(n.id)} className="bg-red-500 text-white px-4 py-2 rounded flex items-center gap-1"><Trash2 size={16}/> Delete</button>
                  </div>
                </div>
              ))}
            </div>
